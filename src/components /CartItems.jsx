  ...
  return (
   <section className='max_padd_container pt-40'>
    <table className='w-full mx-auto'>
        <thead>
            <tr className='bg-slate-900/10 regular sm:regular-22
            text-start py-12'>
        ...
        <tbody>
            {all_products.map((e)=>{
                if(cartItems[e.id]>0){
                    ...
                    border-slate-900/20 text-gray-30 p-6 medium-14 text-center'>
                     <td className='flexCenter'><img src={e.image} alt="prdimg"
                     height={43} width={43} className='rounded-lg ring-1 ring-slate-900/5
                     my-1'/></td>
                    <td><div className='line-clamp-3'>{e.name}</div></td>
                    <td>${e.new_price}</td>
                    <td className='w-16 h-16 bg-white'>{cartItems[e.id]}</td>
                    <td>${e.new_price*cartItems[e.id]}</td>
                    <td><div className='bold-22 pl-14 text-gray-50'> 
