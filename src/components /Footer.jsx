...
const Footer = () => {
 ...
 <footer className="flexCenter pb-24 pt-20">
  <div className="max_padd_container flex w-full flex-col gap-14">
    ...
    md:flex-row">
      <Link to="/" className="mb-10 text-gray-90 bold-20">Ava Chikan</Link>
      <div className="flex flex-wrap gap-8 sm:justify-between md:flex-1">
        ...
        {FOOTER_LINKS.map((col)=>(
          <FooterColumn title={col.title} key={col.title}>
            <ul className="flex flex-col gap-4 regular-14 text-gray-20 ">
              {col.links.map((link)=>(
                <Link className="text-gray-20" to="/" key={link}>{link}</Link>
              ))}
            </ul>
          </FooterColumn>
        ))  }
        ...
        <ul className="flex-gap-4"> 
          {SOCIALS.links.map((link)=>(
            <Link to="/" key={link} className="flex "><img src={link}  alt="socialIcon"
    ...
    <p className="text-center regular-14 text-gray-30">
     2024 Ava Chikan | All Rights Reserved. 
    </p>
  </div>
 </footer>
  )
